...
GiPCordys.scheduler.Scheduler = function(container, options)
{
	...
	var render = function()
	{
		...
		today_button.className = "today_btn";
		...
		previous_button.className = "prev_btn";
		...
		next_button.className = "next_btn";
			...
			popup.appendChild(save_changes);
			  ...
			  save_button.addListener('click', function(e)
			  {
				  ...
				  if(this._options.view=='week'){
						...
						var rec_act=this.getSelectedActivity().newresource.activities;
						var top=0;
						for(var i in rec_act)
						{
						   if(rec_act[i]!=undefined)
						   {
							if(this.activitiesOverlap(this.getSelectedActivity().activity, rec_act[i]))
							{
							top=rec_act[i].getElement().offsetTop+15+4+'px'
							}
						   }
						}
						this.getSelectedActivity().activity.getElement().style.top=top;
						this.getSelectedActivity().activity.getResizer().style.top=top;
...
GiPCordys.scheduler.Scheduler.prototype = {
	...
	displayWeek : function(Start_Year, Start_month, Start_day)
	{
		...
		for(var i=1;i<=7;i++)
		  ...
		  this._elems['times'].removeClass('time_inner');
		  this._elems['times'].addClass('time_inner_week')
	...
	addResource: function(resource)
	{
		if(resource.getId())
		{
			...
			if(index == -1)
			{
				...
				if(this._options.view=="week")
				{
					if((resource.getId()%2)==0)
						resource.getElements().activityHolder.addClass("activities_resource_week week_evenrow");
					else
						resource.getElements().activityHolder.addClass("activities_resource_week week_oddrow");
				}
				else{
					if((resource.getId()%2)==0)
						resource.getElements().activityHolder.addClass("activities_resource evenrow");
					else
					     resource.getElements().activityHolder.addClass("activities_resource oddrow");
				}
	...
	activitiesOverlap: function(Newactivity, Comp_activity)
	{
	    if(Newactivity.getId()!=Comp_activity.getId())
		{
	     if(Newactivity.getStartDate().getTime()>=Comp_activity.getStartDate().getTime())
		 {
		     if(Newactivity.getStartDate().getTime()<=Comp_activity.getEndDate().getTime())
			     return true;
		     else
			     return false;
		 }
		 ...
		 {
			 if(Comp_activity.getStartDate().getTime()<Newactivity.getEndDate().getTime())
			     return true;
		     else
			     return false;
		 }
	   ...
	   if(resource.getId() && activity.getId())
		{
			try 
			{
			//can be merged with addActivity itself.
				...
				if(activity.getStartDate().getDate()!=activity.getEndDate().getDate())
				{
					...
					activity_div.style.width=(daydiff+1)*170-3+"px";
				}
				else
					activity_div.style.width= "167px";

				activity_div.style.left=/*(actperday*25)+*/activity.getStartDate().getDay()*170+1+"px"
				activity_div.style.top=actperday*(15+3)+1+"px";
				
				if(actperday>=2){
					var newWidth=(15+4)*(actperday+1)+"px";
					resource.obj._elements.activityHolder.style.height=newWidth;
					resource.obj._elements.resourceHolder.style.height=newWidth;					
				}
				...
				activity_div.addListener('click',function(e)
				{
					 ...
					 if(activity.getElement().windowLengthWidth().width>=1300)
					    ...
					    sidemargin=90;	 
						this._elems['popup'].style.top=177+toppx+25+"px";
					    this._elems['popup'].style.left=sidemargin+leftpx+35+"px";
					 ...
					 this.setSelectedActivity(activity.getResourceAttached(),activity,resource);
				}.bind(this))
				...
				dact.getDragable().onDragEnd(function(e,activity,resource)
				{
				        ...
				        var leftpx = dact.getDragable().getElem().offset().left;
						var toppx = dact.getDragable().getElem().offset().top+2;
					    ...
					    while(height<toppx)
						{
						    height=height+parseInt(this._resources[i].obj.getElements().activityHolder.offsetHeight);
							++i;
						}
						var r=i-1;
	...
	addActivity: function(resource, activity)
	{
		if(resource.getId() && activity.getId())
		{
			try 
			{
				...
				var resource = this.addResource(resource);
				var rec_act=resource.activities;
				var top=0;
				for(var i in rec_act)
				{
				     if(this.activitiesOverlap(activity, rec_act[i]))
					     {
						   top=rec_act[i].getElement().offsetTop+15+4+"px"
						 }
			    }
				...
				dact.getDragable().onDragEnd(function(e,activity,resource)
				{
				    ...
				    var rec_act=this._resources[r].activities;
					var top=0;
					for(var i in rec_act)
						{
						   if(rec_act[i]!=undefined)
					...
					activity_div.style.top=top;
					activity.getResizer().style.top=top;
	...
	addResizer: function(resource, activity)
	{
			...
			resizer_div.style.top=activity.getElement().offsetTop+"px";
			...
			dact.getDragable().onDragEnd(function(e,activity,resource,r)
			{
				...
				for(var i in rec_act)
					...
					activity.getElement().style.top=top;
					activity.getResizer().style.top=top;
	  ...
	  if(activity.getId())
		{
			...
			activity_div.style.left=left;*/
				...
				activity_div.style.left=/*(actperday*25)+*/activity.getStartDate().getDay()*170+1+"px"
				activity_div.style.top=len*(15+4)+((len==0)?1:0)+"px";
				
				if(len>=2){
					var newWidth=(15+4)*(len+1)+"px";
					resource.obj._elements.activityHolder.style.height=newWidth;
					resource.obj._elements.resourceHolder.style.height=newWidth;					
				}
				activity.setResourceAttached(resource);
	...
	changeActivityResource: function(activity, newResource, oldResource)
	{
		...
		{ 
			...
			for(var k=0;k<tmpArray.length;k++)
			{
				...
				this.updateoldresourceforday(tmpArray[k],oldResource)
			}
			delete tmpArray;				
	...
	updateoldresource: function(activity, resource)
	{
			...
			activity_div.id = "activity_"+activity.getId();
				...
				{
					var daydiff=activity.getEndDate().getDate()-activity.getStartDate().getDate();
					activity_div.style.width=(daydiff+1)*170+"px";
				}
				else
				{
					activity_div.style.width= "167px";
				...
				activity_div.style.left=/*(actperday*25)+*/activity.getStartDate().getDay()*170+1+"px"
				activity_div.style.top=(len-1)*(15+3)+1+"px";
				var newWidth=(len<=2)?"37px":((15+4)*(len)+"px");
				resource.obj._elements.activityHolder.style.height=newWidth;
				resource.obj._elements.resourceHolder.style.height=newWidth;
	},
	
	updateoldresourceforday:function(activity, resource)
	{
	            
				var rec_act=activity.getResourceAttached().activities;
					var top=0;
					for(var i in rec_act)
						{
						   if(rec_act[i]!=undefined)
